...
const ManageSingle = (props) => {
  ...
  const deleteProduct = () => {
    ...
    if (proceed) {
      fetch(`https://bloom-lips-server.onrender.com/products/${_id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
      ...
      <div className="p-6 rounded-md shadow-md bg-coolGray-50 text-coolGray-900 h-full">
        ...
        <div className="mt-6 mb-2">
          <h1 className="font-semibold">
            Color:
            <i className="fas fa-bahai text-lg ml-2" style={{color: `${color}`}}></i>
          </h1>
          <h2 className="lg:text-xl text-md font-semibold tracking-wide">{name}</h2>
        </div>
        <div className="text-right">
          <button className="py-2 px-3 rounded-full bg-pink-300" onClick={deleteProduct}>
